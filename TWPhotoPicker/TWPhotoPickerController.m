    ...
    ALAssetsLibraryGroupsEnumerationResultsBlock listGroupBlock = ^(ALAssetsGroup *group, BOOL *stop) {
        ...
        if (group == nil) {
            if (self.assets.count) {
                UIImage *image = self.imageToCrop ? : [self __imageForAsset:[self.assets objectAtIndex:0]];
...
- (UIView *)topView {
    if (_topView == nil) {
        ...
        backBtn.frame = CGRectMake(0, 0, 80, CGRectGetHeight(navView.bounds));
        ...
        [cropBtn setTitleColor:[[UIColor cyanColor] colorWithAlphaComponent:0.7] forState:UIControlStateHighlighted];
        [cropBtn addTarget:self action:@selector(cropAction) forControlEvents:UIControlEventTouchUpInside];
        [navView addSubview:cropBtn];
        
        self.okButton = cropBtn;
...
- (void)animate{
    ...
    UIView *targetSuperview = [self.okButton superview];
    CGRect targetFrame = self.okButton.frame;
    
    UIActivityIndicatorView *activity = [[UIActivityIndicatorView alloc] initWithFrame:targetFrame];
    activity.activityIndicatorViewStyle = UIActivityIndicatorViewStyleWhite;
    [targetSuperview addSubview:activity];
...
{
    UIImage *image = [self __imageForAsset:[self.assets objectAtIndex:indexPath.row]];
    [self.imageScrollView displayImage:image];
...
- (UIImage *)__imageForAsset:(ALAsset *)asset{
    ALAssetRepresentation *imageRep = [asset defaultRepresentation];
    return  [UIImage imageWithCGImage:imageRep.fullResolutionImage
                                scale:imageRep.scale
                          orientation:imageRep.orientation];
}
